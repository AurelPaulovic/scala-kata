package com.aurelpaulovic.scala_kata.s_99.p10
...
class P10 extends UnitSpec {
	val funcs: List[(String, List[Any] => List[(Any, Int)])] = List( // we could use type Any, but than we would have to assert using == ... I think there is no way to specify it better :(
			("encode", encode(_))
		)
	...
	for((k, f) <- funcs) {
	  k should "return a run-length encoding of a list with consecutive duplicate elements" in {
	    ...
	    assert(f(list) === List((1, 2), (2, 3), (3, 1), (5, 1), (1, 2), (2, 1), (4, 1)))
	  }
	}
	...
	for((k, f) <- funcs) {
	  k should "return a list of single elements packed in run-length tuples of length 1 for a list with no consecutive duplicate elements" in {
	    ...
	    assert(f(list) === List((1, 1), (4, 1), (7, 1), (8, 1), (4, 1), (1, 1)))
	  }
	}
}